...
// Rev 48db.49: client sub-component for the insights listing page.
// Receives SSR-fetched articles as a prop; owns category/search UI
// state only. Articles already render in SSR HTML above — this layer
// just hides/shows + filters after hydration so crawlers see the
// full list without JS, visitors get interactive filtering with JS.
...
export default function InsightsListingClient({ initialArticles }) {
  ...
  const hasNoArticles = initialArticles.length === 0;
      ...
      {hasNoArticles && (
        <div className="py-16">
          <div className="mx-auto max-w-7xl px-6 lg:px-8 text-center">
            <p className="text-sm text-[var(--color-text-muted)]">No articles are available right now. Please check back shortly.</p>
          </div>
        </div>
      )}
